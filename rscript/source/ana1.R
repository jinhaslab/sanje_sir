...
iga_popu %>%
  left_join(iga_dz, by = c("gender", "new_code", "age_gp")) %>%
  left_join(gasRate, by = c("gender", "age_gp")) %>%
  mutate(across(c("n"), ~ifelse(is.na(.x), 0, .x))) %>%
  mutate(expected = pop * gasRate) %>%
  group_by(gender, new_code) %>%
  mutate(Tobs = sum(n), Lobs = exactPoiCI(Tobs)$lower, Uobs = exactPoiCI(Tobs)$upper,
         Texp = sum(expected), 
         SIR  = Tobs/Texp, 
         LL   =  Lobs/Texp,
         UL   =  Uobs/Texp )  %>%
  mutate(SIR95CI = sprintf("%0.2f (%0.2f-%0.2f)", SIR, LL, UL)) -> iga_ratio

iga_ratio %>%
  select(gender, new_code, Tobs, SIR, LL, UL, SIR95CI) %>%
  unique() -> iga_ratio_smry



# Create the plot
fig1 = iga_ratio_smry %>%
  ggplot( aes(x = Tobs, y = SIR, ymin = LL, ymax = UL )) +
  geom_point() +
  ...
  geom_label_repel(aes(label = new_code), 
...
out = list(iga_ratio, iga_ratio_smry, fig1)
}