...
siga_pyramid = function(x){

  siga_popu %>%
    #filter(new_code %in% c("A", "L")) %>%
    ggplot(aes(x = age_gp, y = ifelse(gender == 2, -pop, pop), fill = gender)) +
    ...
    facet_grid(new_code ~ size_gp) +
    labs(y = "Population (thousands)", x = "Age Group", fill = "Gender", title = "Population Pyramid") +
